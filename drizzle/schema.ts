import { pgTable, foreignKey, pgPolicy, bigint, smallint, varchar, timestamp, bigserial, text, boolean, check, pgEnum } from "drizzle-orm/pg-core"
...
export const sections = pgTable("sections", {
	...
	id: bigint({ mode: "number" }).generatedByDefaultAsIdentity({ name: "sections_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	pgPolicy("Section access by granular role", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(EXISTS ( SELECT 1
...
export const allowedUserSections = pgTable("allowed_user_sections", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const attendances = pgTable("attendances", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).generatedByDefaultAsIdentity({ name: "attendances_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	...
	pgPolicy("Enable read access based on students", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(EXISTS ( SELECT 1
   FROM students
  WHERE (attendances.student_id = students.id)))` }),
	pgPolicy("Enable insert based on students", { as: "permissive", for: "insert", to: ["authenticated"] }),
]);
...
export const allowedUsers = pgTable("allowed_users", {
	...
	pgPolicy("Enable read access for its own account", { as: "permissive", for: "select", to: ["authenticated"], using: sql`((((( SELECT auth.jwt() AS jwt) -> 'user_metadata'::text) -> 'allowed_user_id'::text))::integer = id)` }),
]);

export const students = pgTable("students", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).generatedByDefaultAsIdentity({ name: "students_lrn_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	...
	pgPolicy("Student access by granular role", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(EXISTS ( SELECT 1
   FROM allowed_users au
  WHERE ((au.id = (((( SELECT auth.jwt() AS jwt) -> 'user_metadata'::text) ->> 'allowed_user_id'::text))::integer) AND (((au.role = 'CLASS'::"STUDENT_ACCESS") AND (EXISTS ( SELECT 1
           FROM allowed_user_sections aus
	...
	check("students_first_name_check", sql`(length(first_name) > 0) AND (length(first_name) <= 50)`),
	check("students_last_name_check", sql`(length(last_name) > 0) AND (length(last_name) <= 32)`),
	check("students_lrn_check", sql`(id >= '100000000000'::bigint) AND (id <= '999999999999'::bigint)`),
	check("students_middle_name_check", sql`length(middle_name) <= 16`),
]);

export const schools = pgTable("schools", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).generatedByDefaultAsIdentity({ name: "schools_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	name: text().notNull(),
	domain: text(),
	decryptionKey: text("decryption_key").default(sql`replace(replace(encode(extensions.gen_random_bytes(32), \'base64\'::text), \'+\'::text, \'-\'::text), \'/\'::text, \'_\'::text)`).notNull(),
}, (table) => [
	foreignKey({
	...
	pgPolicy("School access by domain", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(EXISTS ( SELECT 1
   FROM allowed_users au
